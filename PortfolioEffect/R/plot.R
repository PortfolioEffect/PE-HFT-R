...
#`+.portfolioPlot` <- function(e1, e2){
#	if(class(e2)[1]=="portfolioPlot"){
#		e1=portfolioPlotPlusPortfolioPlot(e1,e2)
#	}else{
#		print(show(e1)+e2)
#	}
#	return(e1)
...
#plot_3d<-function(metric){
#	normTime<-as.POSIXlt(metric[,1]/1000,origin = "1970-01-01",tz="America/New_York")
#	TimeStep<-which(c(0,diff(normTime$wday))!=0)
#	days<-array(1,dim=TimeStep[1])
#	for(i in 2:length(TimeStep)){
#		days<-c(days,array(i,dim=(TimeStep[i]-TimeStep[i-1])))
#	}
##	result<-cbind(format(normTime,"%H:%M:%S"),metric[,2],days)
#	profit<-metric[,2]
#	seconds<-normTime$hour*60*60+normTime$min*60+normTime$sec
#	plot3d(seconds,profit,days)
#}

#plot_3d<-function(metric,resultsSamplingSec,nameMetric){
#	normTime<-as.POSIXlt(metric[,1]/1000,origin = "1970-01-01",tz="America/New_York")
#	x<-seq(0,23400,by=resultsSamplingSec)
#	TimeStep<-which(c(0,diff(normTime$wday))!=0)
#	n<-length(TimeStep)
#	y<-1:n
#	days<-array(1,dim=(TimeStep[1]-1))
#	for(i in 2:n){
#		days<-c(days,array(i,dim=(TimeStep[i]-TimeStep[i-1])))
#	}
#	days<-c(days,n)
#	z<-matrix(NA,ncol=length(y),nrow=length(x))
##	sec<-(normTime$hour*60*60+normTime$min*60+normTime$sec)
##	ymd<-format(normTime,"%Y/%m/%d")
#	for(i in y){
#		z[1:length(metric[days==i,2]),i]=metric[days==i,2]
##		print(i)
#	}
##zlim <- range(y)
...
#	
##open3d()
##surface3d(x, y*1000, z*100000000, color=col, back="lines", front="lines")
##axes3d( edges=c("x--", "y--", "z"), color=rgb(0.4, 0.4, 0.4) )
#	
#	palette <- colorRampPalette(c("blue", "green", "yellow", "red")) 
#	col.table <- palette(256)
#	col.ind <- cut(z, 256)
#	persp3d(x, y, z, col=col.table[col.ind],axes=FALSE,title=FALSE,xlab = "Seconds", ylab = "", 
#			zlab = nameMetric,main="Variance")
#	axes3d(c('x','z'),nticks=7)  
#	axis3d(edge= 'y+-', at=as.integer(seq(1,n,len=7)), labels = format(normTime[TimeStep[as.integer(seq(1,n,len=7))]],"%m/%d"),nticks=7)
#	
#	
...
#	diff<-difftime(as.POSIXlt(tail(value[,1],1)/1000,origin = "1970-01-01",tz="America/New_York"),
#			as.POSIXlt(value[1,1]/1000,origin = "1970-01-01",tz="America/New_York"),units="days")
#	diffNum<-which.max(diff> c(3*365,365,90,20,2,0.1,0.03,0.015,0.008,0.0025,0.001,0))
#	resultsSamplingSec<-switch(diffNum,
#			"1"= 23400,
#			"2"= 23400/4,
#			"3"= 60,
#			"4"= 20,
...
#	result<-data.frame(data=value[,2],time=1:n)
	
#	p1<-ggplot(data=result, aes(x=time,y=data)) + geom_line(col="#004A61",size=1.5,fill="#004A61") +
#			xlab(NULL) + 
#			ylab(NULL) +
#			scale_x_continuous(breaks=TimeStep,
#					labels=format(normTime[TimeStep],format))+
#			ggtitle("Portfolio value ($)")+util_plotTheme(axis.text.size=1.5)
#	
#	result5<-data.frame(data=portfolio_expectedReturn(portfolio)[,2],time=1:n)
#	
#	p4<-ggplot(data=result5, aes(x=time,y=data)) + geom_line(col="#004A61",size=1.5,fill="#004A61") +
#			xlab(NULL) + 
#			ylab(NULL) +
#			scale_x_continuous(breaks=TimeStep,
#					labels=format(normTime[TimeStep],format))+
#			geom_hline(yintercept=0,col='red',size=0.5)+
#			#   coord_trans(x="one_over") +
#			ggtitle("Portfolio Expected Return")+util_plotTheme(axis.text.size=1.5)
#	
#	
#	result6<-data.frame(data=portfolio_variance(portfolio)[,2],time=1:n)
#	
#	p5<-ggplot(data=result6, aes(x=time,y=data)) + geom_line(col="#004A61",size=1.5,fill="#004A61") +
#			xlab(NULL) + 
#			ylab(NULL) +
#			scale_x_continuous(breaks=TimeStep,
#					labels=format(normTime[TimeStep],format))+
#			#   coord_trans(x="one_over") +
#			ggtitle("Portfolio Variance")+util_plotTheme(axis.text.size=1.5)
